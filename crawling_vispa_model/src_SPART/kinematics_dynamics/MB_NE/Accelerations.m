function [t0dot,tLdot]=Accelerations(t0,tL,P0,pm,Bi0,Bij,x0_dot,q_dot,x0_ddot,q_ddot,robot)
% Computes the operational-space accelerations (twist-rate) of the multibody system.
%
% [t0dot,tLdot]=Accelerations(t0,tL,P0,pm,Bi0,Bij,x0_dot,q_dot,x0_ddot,q_ddot,robot)
%
% :parameters: 
...
%   * x0_dot -- Base-link velocities [\omega,rdot]. The angular velocity is projected in the body-fixed CCS, while the linear velocity is projected in the inertial CCS -- [6x1].
%   * q_dot -- Joint velocities -- [n_qx1].
%   * x0_ddot -- Base-link accelerations [\omegadot,rddot]. The angular acceleration is projected in a body-fixed CCS, while the linear acceleration is projected in the inertial CCS -- [6x1].
%   * q_ddot -- Manipulator joint accelerations -- [n_qx1].
...
%--- Twist Rate ---%
%Base-link
t0dot = Omega0*P0*x0_dot+P0*x0_ddot;
...
for i=1:n
    ...
    if robot.joints(i).type~=0
        tLdot(1:6,i)=tLdot(1:6,i)+Omegam(1:6,1:6,i)*pm(1:6,i)*q_dot(robot.joints(i).q_id)+pm(1:6,i)*q_ddot(robot.joints(i).q_id);
    end
