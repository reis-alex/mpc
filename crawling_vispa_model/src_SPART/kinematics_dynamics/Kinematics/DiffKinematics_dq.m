function [dBij_dql,dBi0_dql,P0,dpm_dql]=DiffKinematics_dq(dTL, dTJ, R0,k,g,ind_q_l,robot)
...
[RJ_ql,RL_ql,rJ_ql,rL_ql,k_ql,g_ql]  = dT_parser(dTJ, dTL, robot, ind_q_l, R0);
%Pre-allocate Bij
dBij_dql=zeros(6,6,n,n,'like',R0);
...
for j=1:n
    for i=1:n
        if robot.con.branch(i,j)==1
            ...
            dBij_dql(1:6,1:6,i,j)=[zeros(3), zeros(3,3); SkewSym(rL_ql(1:3,j)-rL_ql(1:3,i)), zeros(3)];
        else
            %Links are not in the same branch
            dBij_dql(1:6,1:6,i,j)=zeros(6,6);
        end
    end
end

%Pre-allocate Bi0
dBi0_dql=zeros(6,6,n,'like',R0);

%Compute Bi0
for i=1:n
    dBi0_dql(1:6,1:6,i)=[zeros(3), zeros(3,3); SkewSym(-rL_ql(1:3,i)), zeros(3)];
end

%--- Twist-propagation "vector" ---%

%Pre-allocate pm
dpm_dql=zeros(6,n,'like',R0);

%Base-link
P0=zeros(6,6,"like",R0);%[R0,zeros(3,3); zeros(3,3), eye(3)];

%Forward recursion to obtain the twist-propagation "vector"
for i=1:n
    if robot.joints(i).type==1
        %Revolute joint
        dpm_dql(1:6,i)=[k_ql(1:3,i);cross(k_ql(1:3,i),g(1:3,i))+ cross(k(1:3,i),g_ql(1:3,i))];
    elseif robot.joints(i).type==2
        %Prismatic joint
        dpm_dql(1:6,i)=[zeros(3,1);k_ql(1:3,i)];
    elseif robot.joints(i).type==0
        %Fixed joint
        dpm_dql(1:6,i)=zeros(6,1);
    end
end
